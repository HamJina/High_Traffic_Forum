package high.traffic.forum.hotarticle.repository;
...
public class HotArticleListRepository {
    ...
    public void add(Long articleId, LocalDateTime time, Long score, Long limit, Duration ttl) {
        // 네트워크 통신을 redis쪽으로 한 번만 연결하면서 여러개 연산을 한 번에 수행
        redisTemplate.executePipelined((RedisCallback<?>) action -> {
            StringRedisConnection conn = (StringRedisConnection) action;
            String key = generateKey(time); // 시간으로 키 생성
            conn.zAdd(key, score, String.valueOf(articleId)); // sortedset을 사용하면 앞에 z가 붙은 메소드를 사용하면 된다.
            conn.zRemRange(key, 0, - limit - 1); // limit 개수만큼만 sorted set에 데이터 유지
